...
// validate auto-predictions up to 15m
  ...
  // =========================
  // 🔇 Debug helpers (rate-limited)
  // =========================
  _dbgLast: {},
  dbg(tag, ...args) {
    if (!this.config || !this.config.debugMode) return;
    try {
      const now = Date.now();
      const key = String(tag || 'log');
      const last = this._dbgLast[key] || 0;
      // default: at most 1 log / 5s per tag
      if (now - last < 5000) return;
      this._dbgLast[key] = now;
      console.log(...args);
    } catch (_) { /* no-op */ }
  },
  dbgAlways(...args) {
...
const proPrediction = {
    ...
    // Enforce pending queue guardrail (avoid unbounded growth)
    const maxPending = this.config.maxPending || 25;
    if (this.predictions.pending.length >= maxPending) {
      // try to drop oldest AUTO prediction first
      const idx = this.predictions.pending.findIndex(p => p && p.isAuto);
      if (idx >= 0) this.predictions.pending.splice(idx, 1);
      // if still saturated, drop oldest
      if (this.predictions.pending.length >= maxPending) this.predictions.pending.shift();
    }

    this.predictions.pending.push(proPrediction);
    
    // Schedule verifications (bounded). Auto mode schedules only up to autoMaxHorizon.
    const autoMax = this.config.autoMaxHorizon || 15;
    const horizonsToVerify = proPrediction.isAuto
      ? (this.config.horizons || []).filter(h => h <= autoMax)
      : (this.config.horizons || []);

    // Keep timer handles to allow cleanup on completion
    proPrediction._timers = [];
    for (const horizon of horizonsToVerify) {
      const t = setTimeout(() => this.verifyProPrediction(proPrediction.id, horizon), horizon * 60000);
      proPrediction._timers.push(t);
  ...
  completeProPrediction(pred) {
    ...
    // Cleanup any scheduled timers to avoid long-lived closures/timers
    try {
      if (pred._timers && Array.isArray(pred._timers)) {
        for (const t of pred._timers) { try { clearTimeout(t); } catch (_) {} }
      }
      pred._timers = [];
    } catch (_) { /* no-op */ }
  ...
  // ============================================
  startAutoPrediction() {
    // Prevent duplicate loops (hot reload / re-init safety)
    if (this._autoPredictIntervalId) return;

    const AUTO_PREDICT_INTERVAL = 60000; // 1 minute

    this._autoPredictIntervalId = setInterval(() => {
      try { this.generateAutoPrediction(); } catch (_) { /* no-op */ }
    }, AUTO_PREDICT_INTERVAL);

    // First prediction after 5 seconds (startup)
    setTimeout(() => {
      this.dbg('autopredict', '[AI-PRO] 🚀 First auto-prediction...');
      try { this.generateAutoPrediction(); } catch (_) { /* no-op */ }
    }, 5000);

    // Second prediction after 15 seconds (warm-up)
    setTimeout(() => {
      this.dbg('autopredict', '[AI-PRO] 🚀 Second auto-prediction...');
      try { this.generateAutoPrediction(); } catch (_) { /* no-op */ }
    }, 15000);

    this.dbg('autopredict', '[AI-PRO] Auto-prediction started (every 1 min)');
  },
  ...
  generateAutoPrediction() {
    ...
    this.dbg('autopredict', '[AI-PRO] === Auto-prediction attempt ===');
    
    if (!this.isReady) {
      this.dbg('autopredict', '[AI-PRO] ❌ Skipping: not ready');
      return;
    }
    
    const price = window.state?.price;
    if (!price) {
      this.dbg('autopredict', '[AI-PRO] ❌ Skipping: no price data');
      return;
    }
    
    this.dbg('autopredict', '[AI-PRO] ✓ Price available:', price);
    ...
    if (this.predictions.pending.length >= maxPending) {
      this.dbg('autopredict', '[AI-PRO] ⏸ Skipping: too many pending (' + this.predictions.pending.length + '/' + maxPending + ')');
      return;
    }
const marketState = this.getCurrentMarketState();
    this.dbg('market', '[AI-PRO] Market state:', {
    ...
    this.dbg('ensemble', '[AI-PRO] Ensemble result:', {
    ...
    if (ensemble.confidence < 0.15) {
      this.dbg('autopredict', '[AI-PRO] ❌ Skipping: extremely low confidence');
      return;
    }
