...
const OKRT_CONTINENT_ORDER = ['África','América','Asia','Europa','Oceanía'];
  ...
  function installFlagSelector(){
    ...
    shell.innerHTML = '<button type="button" class="okrt-region-btn" aria-haspopup="listbox" aria-expanded="false"><span class="okrt-region-left"></span><span class="okrt-region-chevron">▾</span></button><div class="okrt-region-menu" role="listbox"></div>';
    ...
    menu.innerHTML = groups.map(group => '<div class="okrt-region-group">'+(group.label ? '<div class="okrt-region-group-title">'+group.label.replace(/─/g,'').trim()+'</div>' : '') + group.options.map(opt => '<button type="button" class="okrt-region-item" data-value="'+opt.value+'">'+buildFlag(opt.value, opt.text)+'<span class="okrt-region-name">'+opt.text+'</span></button>').join('') + '</div>').join('');
    function sync(){
      const current = select.value || 'WLD';
      const option = select.querySelector('option[value="'+current+'"]');
      const text = cleanLabel(option ? option.textContent : current);
      left.innerHTML = buildFlag(current, text) + '<span class="okrt-region-name">'+text+'</span>';
  ...
  function installDataBlock(){
    ...
    card.innerHTML = '<div class="okrt-data-head"><div class="okrt-data-title">DATOS RECOGIDOS</div><div class="okrt-data-region" id="okrt-data-region">—</div></div><div class="okrt-data-grid">'
