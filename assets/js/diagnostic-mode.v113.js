(function(){
  ...
  function getStorageContractInfo(){
    const meta = readJSON('aether_contract_meta');
    const contract = window.AETHER_STORAGE_CONTRACT && window.AETHER_STORAGE_CONTRACT.getDebug
      ? window.AETHER_STORAGE_CONTRACT.getDebug()
      : null;
    return {
      version: contract && contract.version ? contract.version : (meta && meta.version ? meta.version : 'n/d'),
      meta,
      normalizations: contract && contract.normalizations ? contract.normalizations.slice(-5) : []
    };
  }

  function getDependencyRecoveryInfo(){
    const s = window.AETHER_DEPENDENCY_STATE || null;
    if (!s) return null;
    return {
      attempts: Array.isArray(s.attempts) ? s.attempts.slice(-6) : [],
      recovered: Array.isArray(s.recovered) ? s.recovered.slice(-6) : [],
      unavailable: Array.isArray(s.unavailable) ? s.unavailable.slice(-6) : []
    };
  }

  function getFeedHints(){
    const out = [];
    if (state.page === 'signal') {
      out.push({ name:'items visibles', value: document.querySelectorAll('#feed .item, #feed .feed-item, .feed-item').length || 0 });
      out.push({ name:'detalle activo', value: !!document.getElementById('detail') });
    }
    if (state.page === 'globe') {
      out.push({ name:'canvas', value: document.querySelectorAll('canvas').length || 0 });
      out.push({ name:'arcos visibles', value: document.querySelectorAll('#arc-controls .seg.active, #arc-controls .active').length || 0 });
    }
    if (state.page === 'vector') {
      out.push({ name:'svg', value: document.querySelectorAll('svg').length || 0 });
      out.push({ name:'rango umbral', value: !!(document.getElementById('threshold') || document.querySelector('input[type="range"]')) });
    }
    return out;
  }
  ...
  function diagnostics(){
    ...
    return {
      ...
      deps: depChecks(),
      dependencyRecovery: getDependencyRecoveryInfo(),
      checks: pageChecks(),
      feedHints: getFeedHints(),
      perf: getPerf(),
      runtime,
      storageContract: getStorageContractInfo(),
      localStorage: lsKeys,
  ...
  function rowsFromChecks(items){
    return items.map(x => '<div class="aether-diag-row"><span>'+escapeHtml(x.name)+'</span>'+badge(!!x.ok)+'</div>').join('');
  }

  function rowsFromMeta(items){
    return items.map(x => '<div class="aether-diag-row"><span>'+escapeHtml(x.name)+'</span><span class="aether-diag-meta">'+escapeHtml(String(x.value))+'</span></div>').join('');
  }
  ...
  function render(){
    ...
    const storageMetaRows = rowsFromMeta([
      { name:'versión', value:d.storageContract.version },
      { name:'normalizaciones recientes', value:d.storageContract.normalizations.length },
      { name:'meta actualizada', value:(d.storageContract.meta && d.storageContract.meta.updatedAt) ? d.storageContract.meta.updatedAt : 'n/d' }
    ]);
    const storageNorms = d.storageContract.normalizations.length
      ? d.storageContract.normalizations.map(x => '<div class="aether-diag-log">'+escapeHtml(x.key)+' · '+escapeHtml(String(x.before))+' → '+escapeHtml(String(x.after))+'</div>').join('')
      : '<div class="aether-diag-empty">Sin reparaciones recientes</div>';
    const depRecovery = d.dependencyRecovery
      ? ('<h4>Resiliencia</h4>' +
          rowsFromMeta([
            { name:'intentos', value:d.dependencyRecovery.attempts.length },
            { name:'recuperadas', value:d.dependencyRecovery.recovered.length },
            { name:'no disponibles', value:d.dependencyRecovery.unavailable.length }
          ]) +
          (d.dependencyRecovery.recovered.length
            ? d.dependencyRecovery.recovered.map(x => '<div class="aether-diag-log">'+escapeHtml(x.label)+' ← '+escapeHtml(x.url)+'</div>').join('')
            : '<div class="aether-diag-empty">Sin fallbacks activados</div>'))
      : '<div class="aether-diag-empty">Sin datos de resiliencia en esta página</div>';
    const feedHints = d.feedHints.length ? rowsFromMeta(d.feedHints) : '<div class="aether-diag-empty">Sin métricas específicas en esta página</div>';
    ...
    panel.innerHTML = '' +
      ...
      '<div class="aether-diag-grid">' +
        ...
        '<section class="aether-diag-card"><h3>Dependencias</h3>'+deps+depRecovery+'</section>' +
        '<section class="aether-diag-card"><h3>Chequeos página</h3>'+checks+'<h4>Indicadores</h4>'+feedHints+'</section>' +
        '<section class="aether-diag-card"><h3>LocalStorage</h3>'+ls+'<h4>Contrato</h4>'+storageMetaRows+storageNorms+'</section>' +
