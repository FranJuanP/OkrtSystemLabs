(function(){
  ...
  function render(){
    ...
    list.innerHTML = state.moduleTargets.map(item => `
    ...
    dock.querySelector('#aether-health-events').innerHTML = ev.length ? ev.map(e => `<div class="aether-health-event">${new Date(e.t).toLocaleTimeString()} · ${escapeHtml(e.msg)}</div>`).join('') : '<div class="aether-health-event">Sin incidencias críticas. Sistema estable.</div>';
  }
