...
(function(){
  ...
  function mountSafeHTML(el, html){ if (!el) return; const tpl = document.createElement('template'); tpl.innerHTML = String(html || ''); window.AETHERDomGuard?.sanitizeElementTree?.(tpl.content); el.replaceChildren(...tpl.content.childNodes); }
  function setHTML(id, html){ const el = $(id); if (el) mountSafeHTML(el, html); }
  ...
  function renderWatchlist(){
    ...
    if (!items.length) {
      mountSafeHTML(box, `<div class="aether-watch-empty">Añade regiones clave para volver a ellas en un clic, seguir su score, dominante y confianza, y detectar cambios de presión.</div>`);
      return;
    }
    mountSafeHTML(box, items.map(item => {
